"""ÇŞB YFK Elektrik Tesisat B.F.A. PDF parser (2026 format)."""
...
POZ_BLOCK_CODE = re.compile(r"^35\.\d{3}\.\d{4}$")
...
def parse_block(blk: list[str]) -> dict[str, Any]:
    ...
    if i_totals >= 0:
        ...
        if len(nums) >= 4:
            ...
            # Montajsız kayıtlar: [malzeme, kârsız toplam, birim fiyat]
            mal_isc = nums[1]
            birim = nums[2]
        elif len(nums) == 2:
            mal_isc = nums[0]
            birim = nums[1]
        if mal_isc is not None and birim is not None:
            kar_tutar = round(birim - mal_isc, 2)
...
def parse_elektrik_lines(lines: list[str]) -> dict[str, dict[str, Any]]:
