...
def _part_sort_key(name: str) -> tuple[int, str]:
    m = re.search(r"Part(\d+)_of_(\d+)", name, re.I)
    if m:
        return (int(m.group(2)), int(m.group(1)), name.lower())
    return (999, 999, name.lower())


def resolve_pdf_list(extra: list[str], search_dirs: list[Path]) -> list[Path]:
    if extra:
    ...
    paths = discover_pdfs(CILT1_GLOBS, search_dirs)
    return paths
...
def main() -> None:
    ...
    out, added, prev = write_catalog(parsed, merge_existing=args.merge)
