...
class _ProjectsScreenState extends ConsumerState<ProjectsScreen> {
  ...
  Future<void> _deleteProject(Project p) async {
    final ok = await SJModal.confirm(
      ...
      title: 'Projeyi sil',
      message:
          '${p.name} ile bu projeye ait keşif, talep, teklif ve teslim '
          'kayıtları silinsin mi?',
      confirmLabel: 'Sil',
      destructive: true,
    );
    ...
    ref.read(quotesProvider.notifier).replaceAll(
          ref.read(quotesProvider).where((e) => e.projectId != p.id).toList(),
        );
    ref.read(deliveriesProvider.notifier).replaceAll(
          ref
