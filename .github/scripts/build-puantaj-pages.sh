...
if [[ "$(basename "${REPO_CANDIDATE}")" == "staging-src" ]]; then
  ...
  PREFERRED_SOURCE="${REPO_CANDIDATE}/artifacts/santijet-puantaj"
else
...
if [[ -n "${PREFERRED_SOURCE}" && -d "${PREFERRED_SOURCE}" ]]; then
  ...
  echo "Building Puantaj from staging-src (script host)..."
elif [[ -d "${STAGING_SRC}/artifacts/santijet-puantaj" ]]; then
